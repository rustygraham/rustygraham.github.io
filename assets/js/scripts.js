...
const stickyHidingClass = 'is-hiding';
const stickyShownClass = 'is-shown';
const stickyShowingClass = 'is-showing';
let lastScrollTop = 0;
let isWaiting = false;

header.addEventListener('animationend', (event) => {
    if ((event.animationName === 'slideDown' || event.animationName === 'graham-header-slide-down') && header.classList.contains(stickyClass) && !header.classList.contains(stickyHidingClass)) {
        header.classList.remove(stickyShowingClass);
        header.classList.add(stickyShownClass);
    }

    if (event.animationName === 'graham-header-slide-up' && header.classList.contains(stickyHidingClass)) {
        header.classList.remove(stickyClass, stickyHidingClass, stickyShownClass, stickyShowingClass);
    }
});
    ...
    if (!isWaiting) {
        window.requestAnimationFrame(() => {
            ...
            if (header.classList.contains('is-menu-open')) {
                header.classList.remove(stickyHidingClass, stickyShowingClass);
                if (currentScroll > 0) {
                    header.classList.add(stickyClass, stickyShownClass);
                } else {
                    header.classList.remove(stickyClass, stickyShownClass, stickyShowingClass);
                }
            } else if (currentScroll > lastScrollTop && header.classList.contains(stickyClass)) {
                header.classList.remove(stickyShowingClass);
                header.classList.add(stickyShownClass, stickyHidingClass);
            } else if (currentScroll < lastScrollTop && currentScroll > 0) {
                const wasHiding = header.classList.contains(stickyHidingClass);

                if (!header.classList.contains(stickyClass) || wasHiding) {
                    header.classList.remove(stickyHidingClass, stickyShownClass);
                    header.classList.add(stickyClass, stickyShowingClass);
                }
